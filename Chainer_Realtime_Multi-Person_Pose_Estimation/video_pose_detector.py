...
if __name__ == '__main__':
    ...
    out = cv2.VideoWriter('output.mp4',fourcc, 1, (640,426))
    while(True):
        ret , fr = cap.read()
        if ret==True:
            # inference
            person_pose_array = pose_detector(fr)
            # draw and save image
            fr = draw_person_pose(fr, person_pose_array)
            out.write(fr)
            ...
            break
        print('Saving result into output.avi...')
    cap.release()
    out.release()
